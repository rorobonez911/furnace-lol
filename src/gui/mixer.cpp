  ...
  if (ImGui::Begin("Mixer",&mixerOpen,globalWinFlags|(settings.allowEditDocking?0:ImGuiWindowFlags_NoDocking),_("Mixer"))) {
    if (ImGui::BeginTabBar("MixerView")) {
      if (ImGui::BeginTabItem(_("Mixer"))) {
        ...
        const char* chipViewModes[]={"VU Meter","Oscilloscope"};
        if (ImGui::Combo("##mixerChipView",&settings.mixerChipView,chipViewModes,2)) {
          settingsChanged=true;
        }
        int masterChans=e->getAudioDescGot().outChans;
        if (masterChans<1) masterChans=1;
        auto drawMasterViz=[&](ImDrawList* dl, ImRect bb, bool horizontal) {
          if (settings.mixerChipView==1) {
            int samples=512;
            int needle=e->oscWritePos-samples;
            ImVec2 p0=bb.Min, p1=bb.Max;
            float w=p1.x-p0.x, h=p1.y-p0.y;
            dl->AddRectFilled(p0,p1,ImGui::GetColorU32(uiColors[GUI_COLOR_OSC_BG1]));
            ImGui::PushClipRect(p0,p1,true);
            ImVec2 pts[512];

            float mpeak=0.0f;
            for (int s=0; s<samples; s++) {
              int pos=(needle+s)&0x7fff;
              float v=0.0f;
              for (int c=0; c<masterChans; c++) v+=fabsf(e->oscBuf[c][pos]);
              if (masterChans>1) v/=masterChans;
              if (v>mpeak) mpeak=v;
            }
            float mgain=(mpeak>0.001f)?0.45f/mpeak:0.45f;

            if (horizontal) {
              float midY=p0.y+h*0.5f;
              dl->AddLine(ImVec2(p0.x,midY),ImVec2(p1.x,midY),ImGui::GetColorU32(uiColors[GUI_COLOR_OSC_GUIDE]),dpiScale);
              for (int s=0; s<samples; s++) {
                int pos=(needle+s)&0x7fff;
                float samp=0.0f;
                for (int c=0; c<masterChans; c++) samp+=e->oscBuf[c][pos];
                if (masterChans>1) samp/=masterChans;
                float x=p0.x+w*(float)s/(samples-1);
                float y=midY-samp*mgain*h;
                if (y<p0.y) y=p0.y; if (y>p1.y) y=p1.y;
                pts[s]=ImVec2(x,y);
              }
            } else {
              float midX=p0.x+w*0.5f;
              dl->AddLine(ImVec2(midX,p0.y),ImVec2(midX,p1.y),ImGui::GetColorU32(uiColors[GUI_COLOR_OSC_GUIDE]),dpiScale);
              for (int s=0; s<samples; s++) {
                int pos=(needle+s)&0x7fff;
                float samp=0.0f;
                for (int c=0; c<masterChans; c++) samp+=e->oscBuf[c][pos];
                if (masterChans>1) samp/=masterChans;
                float y=p1.y-h*(float)s/(samples-1);
                float x=midX+samp*mgain*w;
                if (x<p0.x) x=p0.x; if (x>p1.x) x=p1.x;
                pts[s]=ImVec2(x,y);
              }
            }
            dl->AddPolyline(pts,samples,ImGui::GetColorU32(uiColors[GUI_COLOR_OSC_WAVE]),0,dpiScale);
            ImGui::PopClipRect();
          } else {
            drawVolMeterInternal(dl,bb,peak,masterChans,horizontal);
          }
        };
        ...
        if (settings.mixerLayout) {
          ...
          if (settings.mixerStyle==2) {
            ...
            drawMasterViz(ImGui::GetWindowDrawList(),ImRect(pos,pos+size),true);
          ...
          if (settings.mixerStyle==2) {
            ImVec2 pos=ImGui::GetCursorScreenPos();
            drawMasterViz(ImGui::GetWindowDrawList(),ImRect(pos,pos+ImVec2(40*dpiScale,maxY)),false);
            ImGui::PushStyleColor(ImGuiCol_FrameBg,0);
            ImGui::PushStyleColor(ImGuiCol_FrameBgActive,0);
            ImGui::PushStyleColor(ImGuiCol_FrameBgHovered,127<<IM_COL32_A_SHIFT);
            ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding,0);
...
bool FurnaceGUI::chipMixer(int which, ImVec2 size) {
  ...
  auto drawMixerViz=[&](ImDrawList* dl, ImRect bb, bool horizontal) {
    if (settings.mixerChipView==1&&e->chipOscBuf[which]!=NULL) {
      int samples=512;
      int needle=e->oscWritePos-samples;
      ImVec2 p0=bb.Min, p1=bb.Max;
      float w=p1.x-p0.x, h=p1.y-p0.y;
      ...
      ImGui::PushClipRect(p0,p1,true);
      ImVec2 pts[512];
      float* buf=e->chipOscBuf[which];

      float peak=0.0f;
      for (int s=0; s<samples; s++) {
        float v=fabsf(buf[(needle+s)&0x7fff]);
        if (v>peak) peak=v;
      }
      float gain=(peak>0.001f)?0.45f/peak:0.45f;

      if (horizontal) {
        float midY=p0.y+h*0.5f;
        dl->AddLine(ImVec2(p0.x,midY),ImVec2(p1.x,midY),ImGui::GetColorU32(uiColors[GUI_COLOR_OSC_GUIDE]),dpiScale);
        for (int s=0; s<samples; s++) {
          float samp=buf[(needle+s)&0x7fff]*gain;
          float x=p0.x+w*(float)s/(samples-1);
          float y=midY-samp*h;
          if (y<p0.y) y=p0.y;
          if (y>p1.y) y=p1.y;
      ...
      drawVolMeterInternal(dl,bb,e->chipPeak[which],e->getDispatch(which)->getOutputCount(),horizontal);
    }
  };
    ...
    if (settings.mixerLayout) {
      ...
      if (settings.mixerStyle==2) {
        ...
        drawMixerViz(ImGui::GetWindowDrawList(),ImRect(pos,posMax),true);
      ...
      if (settings.mixerStyle==1) {
        ImVec2 pos=ImGui::GetCursorScreenPos(),
              size=ImVec2(ImGui::GetContentRegionAvail().x,ImGui::GetFontSize()+ImGui::GetStyle().FramePadding.y*2.0f);
        ImGui::Dummy(size);
        drawMixerViz(ImGui::GetWindowDrawList(),ImRect(pos,pos+size),true);
      }
    } else {
      if (ImGui::Checkbox("##ChipInvert",&doInvert)) {
        e->song.systemVol[which]=doInvert?-vol:vol;
      ...
      ImVec2 pos=ImGui::GetCursorScreenPos();
      if (settings.mixerStyle==2) {
        drawMixerViz(ImGui::GetWindowDrawList(),ImRect(pos,pos+ImVec2(size.x-vTextWidth,volSliderHeight)),false);
