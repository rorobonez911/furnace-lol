  ...
  if (ImGui::Begin("Settings",&settingsOpen,ImGuiWindowFlags_NoDocking|globalWinFlags,_("Settings"))) {
    ...
    if (ImGui::BeginTabBar("settingsTab")) {
      ...
      CONFIG_SECTION(_("General")) {
        ...
        bool discordEnabledB=settings.discordEnabled;
        if (ImGui::Checkbox(_("Enable Discord Rich Presence"),&discordEnabledB)) {
          settings.discordEnabled=discordEnabledB?1:0;
          settingsChanged=true;
        ...
        if (settings.discordEnabled) {
          char clientIdBuf[64];
          strncpy(clientIdBuf,settings.discordClientId.c_str(),sizeof(clientIdBuf)-1);
          clientIdBuf[sizeof(clientIdBuf)-1]=0;
          ImGui::Text("Client ID:");
          ImGui::SameLine();
          ImGui::SetNextItemWidth(200.0f);
          if (ImGui::InputText("##discordClientId",clientIdBuf,sizeof(clientIdBuf))) {
            settings.discordClientId=clientIdBuf;
            settingsChanged=true;
          }
        }
      ...
      CONFIG_SECTION(_("Num Format")) {
        CONFIG_SUBSECTION(_("Pattern Values"));
        if (ImGui::BeginTable("numFmtTable",3,ImGuiTableFlags_SizingFixedFit)) {
          ...
          ImGui::TableNextColumn(); ImGui::TextUnformatted(_("Hex"));
          ImGui::TableNextColumn(); ImGui::TextUnformatted(_("Dec"));

#define NUM_FMT_ROW(label,field) \
          ImGui::TableNextRow(); \
          ImGui::TableNextColumn(); ImGui::TextUnformatted(label); \
          ImGui::TableNextColumn(); \
          if (ImGui::RadioButton("##"#field"H",settings.field==0)) { settings.field=0; settingsChanged=true; } \
          ImGui::TableNextColumn(); \
          if (ImGui::RadioButton("##"#field"D",settings.field==1)) { settings.field=1; settingsChanged=true; }

          NUM_FMT_ROW(_("Row numbers"),patRowsBase)
          NUM_FMT_ROW(_("Order numbers"),orderRowsBase)
          NUM_FMT_ROW(_("Instruments"),patInsBase)
          NUM_FMT_ROW(_("Volume"),patVolBase)
          NUM_FMT_ROW(_("Effect codes"),patEffBase)
          NUM_FMT_ROW(_("Effect values"),patEffValBase)
#undef NUM_FMT_ROW
          ImGui::EndTable();
        }
        END_SECTION;
      ...
      CONFIG_SECTION(_("Color")) {
        ...
        if (settings.basicColors) {
          if (ImGui::TreeNode(_("Interface"))) {
            ...
            if (ImGui::Checkbox(_("Gamer mode"),&rgbSync)) {
              if (!rgbSync) applyUISettings(false);
            }
            ImGui::Unindent();
        ...
        if (ImGui::TreeNode(_("Spectrum Analyzer"))) {
          UI_COLOR_CONFIG(GUI_COLOR_SPECTRUM_FILL,_("Fill"));
          UI_COLOR_CONFIG(GUI_COLOR_SPECTRUM_OUTLINE,_("Outline"));
          UI_COLOR_CONFIG(GUI_COLOR_SPECTRUM_PEAK,_("Peak dots"));
          ImGui::TreePop();
        }
...
void FurnaceGUI::readConfig(DivConfig& conf, FurnaceGUISettingGroups groups) {
...
#ifndef FLATPAK_WORKAROUNDS
    {
      int sysDialogDefault=SYS_FILE_DIALOG_DEFAULT;
      if (getenv("WAYLAND_DISPLAY")!=NULL) sysDialogDefault=0;
      settings.sysFileDialog=conf.getInt("sysFileDialog",sysDialogDefault);
    }
#endif
  ...
  if (groups&GUI_SETTINGS_FONT) {
    ...
    settings.discordEnabled=conf.getInt("discordEnabled",0);
    settings.discordClientId=conf.getString("discordClientId","");
  }
  ...
  if (groups&GUI_SETTINGS_APPEARANCE) {
    ...
    settings.patInsBase=conf.getInt("patInsBase",0);
    settings.patVolBase=conf.getInt("patVolBase",0);
    settings.patEffBase=conf.getInt("patEffBase",0);
    settings.patEffValBase=conf.getInt("patEffValBase",0);
    settings.mixerChipView=conf.getInt("mixerChipView",0);
...
void FurnaceGUI::writeConfig(DivConfig& conf, FurnaceGUISettingGroups groups) {
  ...
  if (groups&GUI_SETTINGS_FONT) {
    ...
    conf.set("discordEnabled",settings.discordEnabled);
    conf.set("discordClientId",settings.discordClientId);
  }
  ...
  if (groups&GUI_SETTINGS_APPEARANCE) {
    ...
    conf.set("patInsBase",settings.patInsBase);
    conf.set("patVolBase",settings.patVolBase);
    conf.set("patEffBase",settings.patEffBase);
    conf.set("patEffValBase",settings.patEffValBase);
    conf.set("mixerChipView",settings.mixerChipView);
