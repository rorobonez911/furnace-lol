...
void FurnaceGUI::drawPatternNew() {
  ...
  if (ImGui::Begin("Pattern",&patternOpen,globalWinFlags|ImGuiWindowFlags_HorizontalScrollbar|(settings.avoidRaisingPattern?ImGuiWindowFlags_NoBringToFrontOnFocus:0)|((settings.cursorFollowsWheel && !selecting)?ImGuiWindowFlags_NoScrollWithMouse:0),_("Pattern"))) {
    ...
    insCellSize=settings.patInsBase?threeChars:twoChars;
    insCellSize.x+=(float)settings.insCellSpacing*dpiScale;
    volCellSize=settings.patVolBase?threeChars:twoChars;
    volCellSize.x+=(float)settings.volCellSpacing*dpiScale;
    effectCellSize=settings.patEffBase?threeChars:twoChars;
    effectCellSize.x+=(float)settings.effectCellSpacing*dpiScale;
    effectValCellSize=settings.patEffValBase?threeChars:twoChars;
        ...
        for (int j=rowsBegin; j<rowsEnd; j++) {
          ...
          if (e->curSubSong->chanCollapse[i]<3) {
            ...
            if (pat->newData[row][DIV_PAT_INS]==-1) {
              ...
              if (settings.patInsBase) snprintf(id,63,"%3d",pat->newData[row][DIV_PAT_INS]);
              else snprintf(id,63,"%.2X",pat->newData[row][DIV_PAT_INS]);
              int insChars=settings.patInsBase?3:2;
              if (pat->newData[row][DIV_PAT_INS]<0 || pat->newData[row][DIV_PAT_INS]>=e->song.insLen) {
                dl->AddText(pos,ImGui::GetColorU32(uiColors[GUI_COLOR_PATTERN_INS_ERROR]),id,id+insChars);
              } else {
                DivInstrumentType t=e->song.ins[pat->newData[row][DIV_PAT_INS]]->type;
                if (t!=DIV_INS_AMIGA && t!=e->getPreferInsType(i)) {
                  dl->AddText(pos,ImGui::GetColorU32(uiColors[GUI_COLOR_PATTERN_INS_WARN]),id,id+insChars);
                } else {
                  dl->AddText(pos,ImGui::GetColorU32(uiColors[GUI_COLOR_PATTERN_INS]),id,id+insChars);
                }
              }
            }
          }
          ...
          if (e->curSubSong->chanCollapse[i]<2) {
            ...
            if (pat->newData[row][DIV_PAT_VOL]==-1) {
              ...
              if (settings.patVolBase) snprintf(id,63,"%3d",pat->newData[row][DIV_PAT_VOL]);
              else snprintf(id,63,"%.2X",pat->newData[row][DIV_PAT_VOL]);
              dl->AddText(pos,ImGui::GetColorU32(volColors[volColor]),id,id+(settings.patVolBase?3:2));
            }
          }
            ...
            for (int k=0; k<e->curPat[i].effectCols; k++) {
              ...
              if (pat->newData[row][index]==-1) {
                ...
                if (pat->newData[row][index]>0xff) {
                  ...
                  if (settings.patEffBase) {
                    snprintf(id,63,"%3d",data);
                  } else if (pat->newData[row][index]>=0x10 || settings.oneDigitEffects==0) {
                    snprintf(id,63,"%.2X",data);
                  } else {
                    snprintf(id,63," %.1X",data);
                  }
                }
                dl->AddText(pos,effectColor,id,id+(settings.patEffBase?3:2));
              }
              ...
              if (pat->newData[row][indexVal]==-1) {
                ...
                if (settings.patEffValBase) snprintf(id,63,"%3d",(unsigned char)pat->newData[row][indexVal]);
                else snprintf(id,63,"%.2X",pat->newData[row][indexVal]);
                dl->AddText(pos,effectColor,id,id+(settings.patEffValBase?3:2));
              }
            }
          }
