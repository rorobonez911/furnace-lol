...
void DivEngine::nextBuf(float** in, float** out, int inChans, int outChans, unsigned int size) {
  ...
  if (isRunning()) {
    ...
    for (int i=0; i<song.systemLen; i++) {
      ...
      if (chipOscBuf[i]!=NULL) {
        int writePos=(oscWritePos-(int)size+32768)&0x7fff;
        for (unsigned int k=0; k<size; k++) {
          float s=0.0f;
          for (int j=0; j<outs; j++) {
            if (disCont[i].bbOut[j]!=NULL) s+=disCont[i].bbOut[j][k];
          }
          if (outs>0) s/=(outs*32768.0f);
          chipOscBuf[i][(writePos+k)&0x7fff]=s;
        }
      }
    }
  } else {
